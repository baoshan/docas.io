...
(function() {
  ...
  Queue = (function() {
    ...
    Queue.prototype.clear = function(callback) {
      ...
      return this.redis.hlen(this.key('PROCESSING'), function(err, processing) {
        if (err) {
    ...
    Queue.prototype.statistics = function(callback) {
      var _this = this;
      return this.redis.multi().scard(this.key('GROUPS')).hgetall(this.key('STATISTICS')).hlen(this.key('PROCESSING')).llen(this.key('FAILED')).smembers(this.key('BLOCKED')).hlen(this.key('WORKERS')).exec(function(multi_err, multi_res) {
